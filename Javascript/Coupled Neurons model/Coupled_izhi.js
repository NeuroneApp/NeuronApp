function createMatrix() {
  ...
  for (var i = 0; i < matrixSize; i++) {
    ...
    for (var j = 0; j < matrixSize; j++) {
      tableHTML += '<td><input type="number" class="matrixInput" id="cell_' + i + '_' + j + '" value="0"></td>';
    }
    tableHTML += '</tr>';
  }
  tableHTML += '</table>';
...
function generateAdjacencyMatrix() {
    ...
    for (var i = 0; i < matrixSize; i++) {
      adjacencyMatrix[i] = [];
      for (var j = 0; j < matrixSize; j++) {
        adjacencyMatrix[i][j] = parseInt(inputs[index].value);
        index++;
      }
    }
  
    console.log(adjacencyMatrix);
    // Now you can use the adjacencyMatrix for your network operations
  }



// The normal structure of the other js files
